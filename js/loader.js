...
export class Loader {
  ...
  tickProgress() {
    ...
    if (target - this.displayProgress < 0.5) {
      this.displayProgress = target;
    } else {
      this.displayProgress += (target - this.displayProgress) * 0.4;
    }

    const rounded = Math.round(this.displayProgress);
    this.numberEl.textContent = rounded;

    const shouldLight = Math.ceil((this.displayProgress / 100) * this.totalLetters);
